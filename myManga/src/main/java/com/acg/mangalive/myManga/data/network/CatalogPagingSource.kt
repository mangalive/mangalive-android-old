...
import com.acg.mangalive.myManga.data.toMangaCard
...
class CatalogPagingSource @AssistedInject constructor(
    @Assisted private val sortingParametersFavourites: SortingParameters,
    @IoDispatcher private val ioDispatcher: CoroutineDispatcher,
    private val favouritesService: CatalogService,
    ...
    private suspend fun doResponse(params: LoadParams<Long>): LoadResult<Long, MangaCard> {
        val pageNumber = params.key ?: CatalogService.INITIAL_PAGE_NUMBER
        val pageSize = params.loadSize.toLong()

        val response = withContext(ioDispatcher) {
            favouritesService.getPage(
                pageNumber,
                pageSize,
                sortingParametersFavourites.criterion
            )
        }
    ...
    private fun checkResponse(
        pageNumber: Long,
        response: Response<PageResponseDto>
    ...
    @AssistedFactory
    interface Factory {
        fun create(@Assisted sortingParametersFavourites: SortingParameters): CatalogPagingSource
    }
}